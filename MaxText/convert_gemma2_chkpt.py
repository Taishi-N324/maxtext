...
def main(raw_args=None) -> None:
    ...
    for layer_idx in range(0, num_layers, 2):
        ...
        self_attention_local["query"]["kernel"].append(
            params["transformer"][in_layer_name_local]["attn"]["q_einsum"][
                "w"
            ].transpose((1, 0, 2))
            * query_pre_attn_scalar
        )
        self_attention_local["key"]["kernel"].append(
            params["transformer"][in_layer_name_local]["attn"]["kv_einsum"]["w"][
                0
            ].transpose((1, 0, 2))
        )
        ...
        if transpose_gating_einsum:
            layer_weight["mlp_local"]["wi_0"]["kernel"].append(
                np.transpose(
                    params["transformer"][in_layer_name_local]["mlp"]["gating_einsum"][
                        "w"
                    ][0]
                )
            )
            ...
            layer_weight["mlp_local"]["wi_0"]["kernel"].append(
                params["transformer"][in_layer_name_local]["mlp"]["gating_einsum"]["w"][
                    0
                ]
            )
            layer_weight["mlp_local"]["wi_1"]["kernel"].append(
        ...
        layer_weight["mlp_local"]["wo"]["kernel"].append(
            params["transformer"][in_layer_name_local]["mlp"]["linear"]["w"]
        )

        layer_weight["pre_self_attention_norm_local"]["scale"].append(
            params["transformer"][in_layer_name_local]["pre_attention_norm"]["scale"]
            + 1
        )
        ...
        layer_weight["post_self_attention_norm_local"]["scale"].append(
            params["transformer"][in_layer_name_local]["post_attention_norm"]["scale"]
            + 1
        )
        ...
        self_attention_global["query"]["kernel"].append(
            params["transformer"][in_layer_name_global]["attn"]["q_einsum"][
                "w"
            ].transpose((1, 0, 2))
            * query_pre_attn_scalar
        )
        self_attention_global["key"]["kernel"].append(
            params["transformer"][in_layer_name_global]["attn"]["kv_einsum"]["w"][
                0
            ].transpose((1, 0, 2))
        )
        ...
        if transpose_gating_einsum:
            layer_weight["mlp_global"]["wi_0"]["kernel"].append(
                np.transpose(
                    params["transformer"][in_layer_name_global]["mlp"]["gating_einsum"][
                        "w"
                    ][0]
                )
            )
        ...
        layer_weight["mlp_global"]["wo"]["kernel"].append(
            params["transformer"][in_layer_name_global]["mlp"]["linear"]["w"]
        )

        layer_weight["pre_self_attention_norm_global"]["scale"].append(
            params["transformer"][in_layer_name_global]["pre_attention_norm"]["scale"]
            + 1
        )
        ...
        layer_weight["post_self_attention_norm_global"]["scale"].append(
            params["transformer"][in_layer_name_global]["post_attention_norm"]["scale"]
            + 1
        )
    ...
    self_attention_local["query"]["kernel"] = np.array(
        self_attention_local["query"]["kernel"]
    ).transpose((1, 0, 2, 3))
    self_attention_local["key"]["kernel"] = np.array(
        self_attention_local["key"]["kernel"]
    ).transpose((1, 0, 2, 3))
    self_attention_local["value"]["kernel"] = np.array(
        self_attention_local["value"]["kernel"]
    ).transpose((1, 0, 2, 3))
    self_attention_local["out"]["kernel"] = np.array(
        self_attention_local["out"]["kernel"]
    ).transpose((1, 0, 2, 3))

    self_attention_global["query"]["kernel"] = np.array(
        self_attention_global["query"]["kernel"]
    ).transpose((1, 0, 2, 3))
    self_attention_global["key"]["kernel"] = np.array(
        self_attention_global["key"]["kernel"]
    ).transpose((1, 0, 2, 3))
    self_attention_global["value"]["kernel"] = np.array(
        self_attention_global["value"]["kernel"]
    ).transpose((1, 0, 2, 3))
    self_attention_global["out"]["kernel"] = np.array(
        self_attention_global["out"]["kernel"]
    ).transpose((1, 0, 2, 3))

    layer_weight["mlp_local"]["wi_0"]["kernel"] = np.array(
        layer_weight["mlp_local"]["wi_0"]["kernel"]
    ).transpose((1, 0, 2))
    layer_weight["mlp_local"]["wi_1"]["kernel"] = np.array(
        layer_weight["mlp_local"]["wi_1"]["kernel"]
    ).transpose((1, 0, 2))
    layer_weight["mlp_local"]["wo"]["kernel"] = np.array(
        layer_weight["mlp_local"]["wo"]["kernel"]
    ).transpose((1, 0, 2))

    layer_weight["mlp_global"]["wi_0"]["kernel"] = np.array(
        layer_weight["mlp_global"]["wi_0"]["kernel"]
    ).transpose((1, 0, 2))
    layer_weight["mlp_global"]["wi_1"]["kernel"] = np.array(
        layer_weight["mlp_global"]["wi_1"]["kernel"]
    ).transpose((1, 0, 2))
    layer_weight["mlp_global"]["wo"]["kernel"] = np.array(
        layer_weight["mlp_global"]["wo"]["kernel"]
    ).transpose((1, 0, 2))

    layer_weight["pre_self_attention_norm_local"]["scale"] = np.array(
        layer_weight["pre_self_attention_norm_local"]["scale"]
    ).transpose((1, 0))
    layer_weight["pre_ffw_norm_local"]["scale"] = np.array(
        layer_weight["pre_ffw_norm_local"]["scale"]
    ).transpose((1, 0))
    layer_weight["post_self_attention_norm_local"]["scale"] = np.array(
        layer_weight["post_self_attention_norm_local"]["scale"]
    ).transpose((1, 0))
    layer_weight["post_ffw_norm_local"]["scale"] = np.array(
        layer_weight["post_ffw_norm_local"]["scale"]
    ).transpose((1, 0))

    layer_weight["pre_self_attention_norm_global"]["scale"] = np.array(
        layer_weight["pre_self_attention_norm_global"]["scale"]
    ).transpose((1, 0))
    layer_weight["pre_ffw_norm_global"]["scale"] = np.array(
        layer_weight["pre_ffw_norm_global"]["scale"]
    ).transpose((1, 0))
    layer_weight["post_self_attention_norm_global"]["scale"] = np.array(
        layer_weight["post_self_attention_norm_global"]["scale"]
    ).transpose((1, 0))
    layer_weight["post_ffw_norm_global"]["scale"] = np.array(
        layer_weight["post_ffw_norm_global"]["scale"]
    ).transpose((1, 0))
