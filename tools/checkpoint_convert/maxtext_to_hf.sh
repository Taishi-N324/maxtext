...
do
    ...
    case "${model_name}" in
        "gemma3-4b")
            cp -r tools/checkpoint_convert/gemma3_language_config/gemma3-4b/config.json "${BASE_DIR}/checkpoints_hf/${exp_name}/checkpoints/${iter}"
            ;;
        "gemma3-12b")
            cp -r tools/checkpoint_convert/gemma3_language_config/gemma3-12b/config.json "${BASE_DIR}/checkpoints_hf/${exp_name}/checkpoints/${iter}"
            ;;
        "gemma3-27b")
            cp -r tools/checkpoint_convert/gemma3_language_config/gemma3-27b/config.json "${BASE_DIR}/checkpoints_hf/${exp_name}/checkpoints/${iter}"
            ;;
        *)
            echo "Warning: model_name ${model_name} は vocab 調整用の config コピーに未対応です。"
            ;;
    esac
